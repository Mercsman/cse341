...
const getSingleAuthor = async (req, res) => {
    ...
    const result = await mongodb.getDatabase().db('books').collection('authors').find({ _id: userId});
    result.toArray((err, lists) => {
        if (err) {
            res.status(400),json({ message: err });
        }
...
const updateAuthor = async (req, res) => {
    ...
    const response = await mongodb.getDatabase().db('books').collection('authors').replaceOne({ _id: userId}, author);
...
const deleteAuthor = async (req, res) => {
    ...
    const userId = new ObjectId(req.params.id);
    const response = await mongodb.getDatabase().db('books').collection('authors').deleteOne({ _id: userId});
